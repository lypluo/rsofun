...
```{r}
## New code for tc_home implementation:
# Extract mean maximum temperature for all months
df_tc_home <- ingest(
...
# Attach tc_home to siteinfo in drivers
df_drivers_fluxnet2015 <- df_drivers_fluxnet2015 %>%
  
  # Drop siteinfo
  dplyr::select(!siteinfo) %>%
  
  # Update siteinfo with tc_home
  left_join(df_drivers_fluxnet2015 %>% 
  ...
  # Scale flux to per seconds
  mutate(prec = prec / (60*60*24), ppfd = ppfd / (60*60*24)) %>%
    
  # Create nested dataframe again
  group_by(sitename) %>% 
  nest() %>%
  rename(forcing = data) %>% 
  right_join(
...
### Save and Load working benchmark outputs
```{r}
# saveRDS(df_output, "~/data/mscthesis/rsofun/df_backup")
# df_output <- readRDS("~/data/mscthesis/rsofun/df_backup")

left_join(siteinfo_fluxnet2015, df_output %>% unnest()) %>% select(sitename, gs_accl, )
left_join(df_drivers_fluxnet2015 %>% unnest(cols = "forcing"), df_output %>% unnest()) %>% dplyr::slice(sample(10))

left_join(df_drivers_fluxnet2015 %>% unnest(cols = "forcing"), df_output %>% unnest()) 

df_dr <- df_drivers_fluxnet2015 %>% unnest(cols = "forcing") %>% 
  dplyr::select(sitename, date, temp) %>% 
  mutate_at(vars(one_of(c("temp"))), rpmodel::dampen_vec, tau = 40) %>% 
  rename(tgrowth = temp)

test <- left_join(df_dr, df_output %>% unnest() %>% rename(topt = gs_accl) %>% mutate(topt = ifelse(topt < 5, NA, topt)) %>% drop_na())

test <- left_join(df_dr, df_output %>% unnest() %>% rename(topt = gs_accl) %>% drop_na())

test %>% ggplot() +
  geom_smooth(aes(tgrowth, topt)) +
