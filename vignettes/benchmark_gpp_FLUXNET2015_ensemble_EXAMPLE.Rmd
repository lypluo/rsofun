...
```{r}
## New code for tc_home implentation:
...
df_drivers_fluxnet2015 <- df_drivers_fluxnet2015 %>%
  ...
  left_join(df_drivers_fluxnet2015 %>% 
              ...
              left_join(df_tc_home, by = "sitename") %>%
              nest("siteinfo" = !sitename),
            by = "sitename")
...
Calibrate the model.
```{r warning=FALSE, message=FALSE}
df_drivers_fluxnet2015$forcing[[1]]$snowf <- 0 # To be added so that run_pmodel_f_bysite() does not give an error
