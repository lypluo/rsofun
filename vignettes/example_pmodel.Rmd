...
siteinfo <- ingestr::siteinfo_fluxnet2015 %>%
  ...
  ## extracting home temperature from worldclim for mysites (mean maximal temperature of warmest month)
  left_join(ingest(
    dplyr::filter(ingestr::siteinfo_fluxnet2015, sitename %in% mysites),
    source    = "worldclim",
    settings  = list(varnam = c("tmax")),
    dir       = "~/data/worldclim") %>%
      
      ## attach home temperature to data frame
...
## TODO - DONE: extract mean tmax of warmest month from worldclim for each site, see https://stineb.github.io/ingestr/articles/example.html#worldclim-3 -> new column in siteinfo data frame 'tc_home')
```
...
## TODO - DONE: fudge df_drivers for this example
df_drivers$siteinfo[[1]] <- df_drivers$siteinfo[[1]] %>% 
  mutate(tc_home = siteinfo$tc_home)
```
