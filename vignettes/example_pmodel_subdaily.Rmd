---
title: "Example using P-model at sub-daily time step"
output:
  html_document:
    toc: true
    toc_float: true
...
rsofun::df_drivers
```
Convert units of PPFD and precipitation (is now in per seconds).
```{r}
...
```{r}
ggplot() +
  geom_line(data = df, aes(x=date, y=pet), color = 'black') + 
  geom_line(data = df, aes(x=date, y=transp), color = 'tomato') + 
  labs(title = "FR-Pue", subtitle = "SOFUN output")

ggplot() +
  geom_line(data = df, aes(x=date, y=gpp), color = 'black') + 
  labs(title = "FR-Pue", subtitle = "SOFUN output")
```
