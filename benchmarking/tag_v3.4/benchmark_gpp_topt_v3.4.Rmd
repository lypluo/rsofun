...
## Load data
```{r}
load("~/data/rsofun_benchmarking/df_drivers_topt.Rdata")
```
...
## Calibrate model
This is done via Fluxnet only

## Run model
```{r}
df_output <- runread_pmodel_f(
     df_drivers = df_drivers_topt %>%
         mutate(forcing = purrr::map(forcing, ~rename(., rainf = rain, snowf = snow))), # TODO: Fix snowf/snow and rainf/rain naming issues
     params_modl = params_modl, 
     makecheck = TRUE,
     parallel = FALSE
     )
...
```{r}
load("~/data/rsofun_benchmarking/obs_eval_topt.Rdata")
```
